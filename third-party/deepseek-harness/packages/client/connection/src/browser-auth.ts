...
function sessionCookie(name: string, value: string, expiresAt: number, maxAgeSeconds: number): string {
  // The launch URL is opened from TapCanvas at localhost:5175 while the
  // Harness is bound to 127.0.0.1:3080. They are different sites, so a
  // Strict cookie is not included on the 303 redirect that completes the
  // one-time launch-token exchange. Lax keeps CSRF protections for subresource
  // and unsafe cross-site requests while allowing that top-level navigation.
