...
- 当前 `tapcanvas-api` Skill 描述画布内的 owner-scoped 原生工具；影视 Function（`film_image_gen`、`film_video_gen`、`film_video_composite`、`film_ask_human`、`film_file_read/write`、`film_memory_recall`、`film_task_create/update/list/read` 及附件别名）均在 `apps/agents/packages/bundle/web-app/src/tapcanvas-scope.ts` 中直接注册。图片和视频 Function 一次调用即可提交节点并复用既有 Hono 执行器；显式传入顶层 `vendor` 时严格使用指定执行器，省略 `vendor` 时由 Hono 根据已启用模型目录中唯一匹配的 ComfyUI 模型记录解析 `comfyui`，没有本地目录匹配才进入 new-api；不会按模型名称硬编码特判或自动更换模型。`film_video_gen` 的 `workflow_capability` 会原样写入节点并透传为 `extras.workflowCapability`，自定义 ComfyUI 变体必须显式选择；`reference-audio-legacy` 还要求 `extras.mediaInputs` 提供真实、按目录声明数量匹配的图片与音频。多段续写要求显式给出 `continuation_from_node`/`continuation_mode`，先从当前画布真实上游视频抽取尾帧资产，再按首帧或全参考合同提交；MiniMax H3 提交边界同时校验对应的结构化提示词字段，缺段落直接失败。文件与任务 Function 使用当前画布隔离的 Agent Workspace；Harness 运行时用当前画布传入的用户身份生成短期内部委托凭据调用 Hono，模型不会接触 token、MCP 或公共 API 配置。未接入统一执行器的别名会显式返回能力错误，不伪造成功；缺少作用域、委托密钥或执行回执时同样显式失败。公共 `/public/*` 脚本配置不属于模型的工具发现路径。
