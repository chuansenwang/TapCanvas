...
- 当前 `tapcanvas-api` Skill 描述画布内的 owner-scoped 原生工具；影视 Function（`film_image_gen`、`film_video_gen`、`film_video_composite`、`film_ask_human`、`film_file_read/write`、`film_memory_recall`、`film_task_create/update/list/read` 及附件别名）均在 `apps/agents/packages/bundle/web-app/src/tapcanvas-scope.ts` 中直接注册。图片和视频 Function 一次调用即可提交节点并复用既有 Hono 执行器；图片 Function 的顶层 `vendor` 必须显式区分执行器：`comfyui` 直连本地 ComfyUI，`newapi` 或省略值才进入 new-api；只传本地模型名不会自动推断执行器。文件与任务 Function 使用当前画布隔离的 Agent Workspace；Harness 运行时用当前画布传入的用户身份生成短期内部委托凭据调用 Hono，模型不会接触 token、MCP 或公共 API 配置。未接入统一执行器的别名会显式返回能力错误，不伪造成功；缺少作用域、委托密钥或执行回执时同样显式失败。公共 `/public/*` 脚本配置不属于模型的工具发现路径。
