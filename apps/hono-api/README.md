...
- 当前 `/public/tasks`、`/public/draw`、`/public/video`、`/public/vision` 与 `/public/video/understand` 默认通过 `NEW_API_INTERNAL_BASE_URL` + `NEW_API_INTERNAL_TOKEN` 请求 `apps/new-api`；图片任务显式传入 `vendor: comfyui` 时直连 `COMFYUI_BASE_URL`，本地 ComfyUI 模型不经过 new-api 启用列表校验。原生 `film_image_gen` Function 对执行器采用显式选择合同，调用方必须传 `vendor`，不会因省略参数而把本地模型送入 new-api。
- MiniMax H3 的本地 ComfyUI 目录保留已验证的 `MiniMax_H3_Easy.json` 节点图，并登记六个显式输入合同：`text`（T2VA，无媒体）、`first_frame`（I2VA，一张首帧图）、`last_frame`（L2VA，一张尾帧图）、`first_last_frame`（FL2VA，各一张首尾帧图）、`reference`（Ref2VA，普通图片/视频/音频参考）和 `digital_human`（数字人，必须显式请求且同时提供人物图与驱动音频）。Easy 变体的所有图片、音频、视频均通过 `MiniMaxH3EasyMediaLoader.media_state` 注入。本地节点实际只接受 `mode=image|reference|digital_human` 与 `keyframe_role=first|last`，因此 T2VA/I2VA/L2VA/FL2VA 统一映射到 `image`，并通过媒体角色与顺序传递输入；首尾帧与普通参考混用、数字人缺图或缺音频、变体无法唯一匹配时均显式失败，不静默改模式。H3 时长档位为 **1–15 秒（含首尾）**，`defaultDurationSeconds=4` 仅表示默认选项，不代表固定 4 秒；请求 1–15 秒均可提交，超过 15 秒必须显式报错，禁止猜测、截断或改写时长。ComfyUI 分辨率枚举统一使用大写（如 `720P`），目录按本机实际枚举登记 `360P` 至 `1080P` 及 `custom`；输入的 `720p` 会在工作流边界规范化为 `720P`。
- 下载的 `H3至尊全能工作流【鱼佬框架+吃猪侠定制版】.json` 作为 `reference-audio-legacy` 独立能力登记，不替换 Easy 变体；调用方必须显式传入 `extras.workflowCapability=reference-audio-legacy`，并在 `extras.mediaInputs` 中提供与目录声明数量一致的图片和音频。该 API graph 的提示词写入节点 `263.inputs.UNKNOWN`，媒体严格按 `LoadImage`/`LoadAudio` 节点顺序写入；图片、音频数量不符，包含不支持的视频，或仍通过 `referenceImages` 传入素材都会直接失败。工作流中的示例文件名不会被提交使用。
- 本地 ComfyUI H3 任务的历史轮询默认上限为 **30 分钟**（`COMFYUI_POLL_TIMEOUT_MS=1800000`），API 与 Agent Bridge 的默认超时上限也为 30 分钟。提交、媒体上传和历史查询的网络异常会分别暴露端点与底层原因；`fetch failed` 不再被当作无上下文的通用错误。
- 本地 ComfyUI 目录新增 `qwen-image-2.1`（阿里 Qwen Image 2.1 文生图），使用与 H3 相同的提交/轮询/取回链路；它是新账号 `imageModel` 初始偏好。该 API graph 的提示词写入 `TextGenerateLTX2Prompt`（节点 `480`）→ `easy showAnything`（`484`）→ `TextEncodeQwenImage21`（`471`）；不能绑定 471，因为它的 `resolution` 是 `INT(0-4096)`，而通用参数注入会把 `extras.resolution`（如 `2K`）以字符串写入并导致 ComfyUI 校验失败。画幅由 `ResolutionSelector`（节点 `13`）决定，目录通过 `comfyui.workflowVariants[].aspectRatioInputBindings` 把请求画幅显式写入该控件的枚举值（如 `16:9` → `16:9 (Widescreen)`）；`meta.imageOptions.aspectRatioOptions` 只声明能真实映射的档位（`1:1/4:3/3:4/16:9/9:16`）。ComfyUI 会静默忽略未知输入字段，因此缺失绑定会让画布上的比例选择看起来成功而实际不生效；请求画幅不在映射内时显式失败，请求未携带画幅时要求工作流默认值本身仍在支持列表内，否则同样显式失败，不静默沿用未展示的比例。
- 本地 ComfyUI 目录的 `qwen-image-2.1` 同时登记 **动态图编辑** 变体（`id=edit`，`taskKind=image_edit`），参考图数量由请求决定，不再按 1/2/3 张写死变体。该变体的参考图槽位来自 `TextEncodeQwenImage21`（节点 `485`）的 autogrow 输入 `images.image_1`…`images.image_16` 与提示词扩写器 `BatchImagesNode`（节点 `505`）的视觉输入 `images.image0`…`images.image15`；autogrow 槽位是 `IMAGE` 类型，只能接节点输出，因此执行器在运行时按请求数量生成 `LoadImage` 节点并以 `[节点 id, 0]` 连线，同一张参考图被多个槽位复用时只建一个节点。数量契约由 `comfyui.workflowVariants[].referenceImageRange`（本机为 `1–16`）声明，`referenceImageSlots` 声明槽位模板与容量，`referenceImageLoaderNodeIdTemplate` 声明运行时 `LoadImage` 的 id 模板。声明了 `referenceImageSlots` 的变体不得再声明定数 `imageNodeIds`；槽位容量不足以覆盖区间上限、工作流仍残留 `LoadImage` 节点、数量超出区间、运行时 id 与既有节点冲突时均显式失败，不静默降级。`meta.imageOptions.maxReferenceImages=16` 同步声明给前端，画布图片节点的参考图上限与执行时的收集上限都以该目录声明为准（未声明的模型沿用既有上限：普通图片 3 张、角色卡 8 张）。
- `qwen-image-2.1` 图编辑的 **参考图编码分辨率跟随画布输出规模**。`TextEncodeQwenImage21`（节点 `485`）的 `resolution` 语义是「参考图缩放到约 resolution × resolution 像素」的面积等效边长（节点声明 `INT min=0 max=4096 step=32`，`0` 表示保持原图尺寸），原先在工作流里写死 `1280`。目录通过 `comfyui.workflowVariants[].imageResolutionInputBindings` 声明该绑定：`alignTo=32` 必须与节点 step 一致，`megapixelsFromNodeId=13` 指向工作流的 `ResolutionSelector`。因为该工作流的输出尺寸由 `ResolutionSelector`（节点 `13`）经开关节点（`483`）驱动 `EmptyLatentImage`（`480`）决定，`ResolutionSelector` 满足 `width × height ≈ megapixels × 1024²`，所以面积等效边长 = `1024 × sqrt(megapixels)`，与画幅比例无关——不需要复制该节点的比例表，也不依赖比例枚举的具体文案。本机 `megapixels=2` 时等效边长 1448，对齐 32 后写入 `1440`；切换画幅比例不改变 `megapixels`，因此编码分辨率保持稳定，仅随输出规模变化，横图与竖图在同一输出规模下得到一致的参考图编码量。请求携带像素尺寸（`imageEdit` 节点路径）时优先按 `width/height` 的几何平均换算；两者都无法确定时保留工作流默认值，但该默认值必须落在声明范围内，否则显式失败，不静默沿用未声明的编码规模。
- `qwen-image-2.1` 图编辑工作流的目录构建在 `apps/hono-api/scripts/build-comfyui-catalog.mjs` 中显式裁剪：导出文件里的 `LoadImage`（节点 `470`，编辑器粘贴图）、`ImageConcatMulti`（`486/487`，对比预览拼图）、重复的 `SaveImageAdvanced`（`496`）与 `SaveImage`（`515`），以及同文件里与本变体无关的另一条文生图链（`516`–`528`）都会被删除，并清理指向已删除节点的悬空连线。这些节点会被 ComfyUI 当作输出节点真实执行并落盘，若保留会让一次图编辑请求产出多张非目标图；裁剪按显式 id 进行，不做猜测式判断。裁剪后 `outputNodeIds` 只保留 `494`。
...
- TapCanvas 领域 Skill 已迁移并受版本控制于 `apps/agents/.agents/skills/tapcanvas-*`，由原生 Harness preset 通过显式 bundled 根目录发现和按需加载；原生 preset 关闭宿主项目根与用户根的默认 Skill 扫描，因此仓库根 `.agents/skills` 不会进入原生 Agent 上下文。`apps/agents-cli/skills/tapcanvas-*` 仅保留为 legacy 迁移源，不能作为当前运行时的 Skill 真源或新功能的维护位置。
- 当前 `tapcanvas-api` Skill 描述画布内的 owner-scoped 原生工具；影视 Function（`film_image_gen`、`film_video_gen`、`film_audio_gen`、`film_video_composite`、`film_ask_human`、`film_media_catalog_get`、`film_file_read/write`、`film_memory_recall`、`film_task_create/update/list/read` 及附件别名）均在 `apps/agents/packages/bundle/web-app/src/tapcanvas-scope.ts` 中直接注册。图片和视频 Function 一次调用即可提交节点并复用既有 Hono 执行器；显式传入顶层 `vendor` 时严格使用指定执行器，省略 `vendor` 时由 Hono 根据已启用模型目录中唯一匹配的 ComfyUI 模型记录解析 `comfyui`，没有本地目录匹配才进入 new-api；不会按模型名称硬编码特判或自动更换模型。`film_audio_gen` 复用 `apps/hono-api/src/modules/task/agents-tool-bridge.generate-audio-to-canvas.ts` 的音频节点执行器（省略 `ai_model` 时只按实时目录声明的 MiniMax H3 语音模型解析，缺声明或存在多个候选时显式失败），参考音色只传当前画布真实音频节点 ID。`film_video_gen` 的 `workflow_capability` 会原样写入节点并透传为 `extras.workflowCapability`，自定义 ComfyUI 变体必须显式选择；`reference-audio-legacy` 还要求 `extras.mediaInputs` 提供真实、按目录声明数量匹配的图片与音频。多段续写要求显式给出 `continuation_from_node`/`continuation_mode`，先从当前画布真实上游视频抽取尾帧资产，再按首帧或全参考合同提交；MiniMax H3 提交边界同时校验对应的结构化提示词字段，缺段落直接失败。文件与任务 Function 使用当前画布隔离的 Agent Workspace；Harness 运行时用当前画布传入的用户身份生成短期内部委托凭据调用 Hono，模型不会接触 token、MCP 或公共 API 配置。未接入统一执行器的别名会显式返回能力错误，不伪造成功；缺少作用域、委托密钥或执行回执时同样显式失败。公共 `/public/*` 脚本配置不属于模型的工具发现路径。
- 模型身份的唯一来源是只读目录工具 `film_media_catalog_get`（Hono 侧 `tapcanvas_media_execution_catalog_get`，能力组 `paid_media_generation`，执行语义 `sideEffect: none`）。它按 `kind` 返回实时可执行的图片/视频/音频目录项与真实物理档位：视频含 `durationOptions`、`resolutionOptions`、`sizeOptions`/`orientationOptions` 推导出的 `aspectRatioOptions`、参考素材上限与 `supportsNativeAudio`；音频含 `audioType` 与 `engine`。数据源与执行器解析 vendor 时查询的**同一张目录表**（`model_catalog_models`，`enabled` 过滤）一致，因此本地执行器模型（如 ComfyUI 的 `minimax-h3`）与系统渠道模型都会出现在目录里；不得改用只覆盖系统渠道的 new-api 运行时投影，否则本地模型会缺失、目录退化为空。`film_video_gen.ai_model` 必须逐字复制该回执的 `video.models[].modelKey`，`duration_sec`/`resolution`/`aspect_ratio` 必须落在同一模型的声明档位内；`film_audio_gen` 只在目录恰好声明一个 `engine=minimax-h3` 的 speech 模型时才允许省略 `ai_model`。目录为空或读取失败时显式失败，禁止回填历史模型名或猜测。视频合片的音轨能力同样走真实执行器：`film_video_composite.audio_list` 的节点 ID 透传为 `tapcanvas_video_concat` 的 `audioNodeIds`（≤3，节点必须已有真实 `audioUrl`），由 `muxAudioOntoVideo` 完成首条替换、其后逐条混音；该执行器与 `concatVideosFromUrls` 一样支持本地资产存储，不再要求必须配置对象存储。
- 当前原生 Agent 的基础 persona 是 TapCanvas 创作 Agent“小T”：负责 AI 视觉创作、视觉策划、画布与资产协作以及项目生产协作。身份层只声明稳定能力和事实优先原则；图片/视频提示词写法、分镜连续性和具体生产方法继续由 `apps/agents/.agents/skills/tapcanvas-*` 按需加载，不固化为 Hono 的常驻 prompt。标准与 PTC preset 会覆盖部署级 persona，因此两者与 Web bundle 使用同一份小T身份文案；模型和媒体能力仍以运行时动态目录及真实工具回执为准。
- `apps/agents` 中的 `@tapcanvas/agents` 是独立的 Harness Runtime 与浏览器 Origin。根 `pnpm dev` 使用 Vite 开发服务器提供 `apps/web` 主页面，同时启动 `apps/agents` 的 `dev:web` watcher；Harness Web 仅提供原生 Agent 工作区，Agent bundle 重建后由内置 HMR 接收器刷新已打开的 iframe。TapCanvas 的小 T 通过 iframe 打开 `/agent/`，开发模式下允许跨 Origin，并通过 `postMessage` 传递结构化画布作用域。跨 Origin 时，父页面在 iframe URL 中显式传入 `parentOrigin`；Agent 端优先使用该值，并以 `document.referrer` 或浏览器 `ancestorOrigins` 作为补充来源，始终同时校验 `event.source === window.parent` 与 `event.origin`。所有来源元数据缺失或无法解析时拒绝作用域消息并保持显式不可用，作用域请求仅在无敏感数据时使用 `*` 发送。生产构建仍可由 Harness Web 托管静态入口；不把 Harness 的文件系统 Workspace 或仓库根目录当作 TapCanvas 画布。父页面通过 `tapcanvas:scope` 结构化消息传递当前项目、Flow、章节、书籍和选中节点 ID，并通过 `tapcanvas:model-catalog` 传递已认证的 `new-api` 动态文本模型目录，Harness 原生会话界面展示该作用域与模型来源。
...
- 画布运行时（`/tapcanvas` RPC 通道、`tool:tapcanvas-native` 提示段、`tapcanvas_get_current_canvas` 与 `film_*` 工具）由 `packages/bundle/web-app` 的 `web-runtime` 行在自身 `inject` 依赖齐备后挂载，因此该挂载的失败是会话级静默的：行内 fiber 失败时不会中断 Host，只是所有原生会话都拿不到画布工具。`ctx.connection.rpc.handle()` 会以 Connection 服务自身的上下文注册路由，所以 Web bundle 必须让 `connection` 行注入 `webServer`，否则通道挂载会以 `cannot get property "webServer" without inject` 失败；这条前置条件由 `apps/agents/apps/cli/tests/tapcanvas-canvas-runtime.spec.ts` 守护。
- 当前原生 Agent 入口以右侧固定栏承载，模型选择器同时显示 Harness 自身配置目录与 TapCanvas `new-api` 目录，两个来源均保持结构化来源标识。父页面同步 `sessionId、projectId、flowId、chapterId、selectedNodeIds` 以及节点/边快照；Host 为每个 `projectId + flowId + chapterId` 生成稳定的隔离画布会话目录，客户端仅在 Session 列表权威快照 ready 后执行作用域同步，并在同步时仅清除仍指向其他目录的恢复会话，再优先保留用户当前选中的同 Workspace 历史会话，否则复用或创建对应 Harness 会话并将其设为当前会话；已经属于当前画布 Workspace 的新会话不会被清除，避免新建对话或点击历史记录出现闪回。嵌入式 Agent 在尚无 Session 时也会先用当前画布作用域创建/复用该 Workspace 下的会话，不展示“选择一个工作区开始”，只有缺少画布作用域时才允许显示无作用域状态。文件沙箱与会话历史不再落到启动目录，也不会加载启动目录中的 `AGENTS.md/CLAUDE.md`。嵌入式 Agent iframe 同时隐藏 Harness 原生目录 Workspace 浏览器和选择器，避免用户误把本地目录当成画布工作区；TapCanvas 画布作用域是用户可见的唯一工作区身份。嵌入模式还把原生侧栏轨道压缩为 `0px`，顶部 Agent 工具栏作为唯一默认入口，用户可通过侧栏按钮临时展开原生浏览器。Host 仅按会话内存保存作用域，不把画布数据写入用户 prompt，也不依赖浏览器 Cookie 或旧 bridge。缺少作用域时原生工具显式失败，不猜测项目或 Flow。

#### 上游发布版同步（2026-09-15）

- `apps/agents` 已从 DeepSeek Harness `0.1.3-alpha.1` 对齐到上游发布 tag `dsh-v0.1.5-rc.2`（跨 1301 个上游提交，逐处人工解决 21 个冲突文件）。本次同步只替换 `apps/agents` 的上游部分；TapCanvas 的 `tapcanvas-*` Skill、`tapcanvas-scope`、`tapcanvas-api-proxy`、画布作用域会话绑定与嵌入式布局约束全部保留，`apps/agents-cli` 与 Agents Bridge 仍为 legacy，未因同步回到默认链路。同步策略见 `docs/adr/0001-harness-native-agent-runtime.md`。
- persona 装配随上游改为两段式：小T 身份文案落在 `system-prompt.personaPrefix`，`personaSuffix` 继续承载 `{{cwd}}`；Web bundle 与 `standard`、`ptc`、`cordis` 三个 preset 使用同一份文案，不再使用上游已移除的单字段 `persona`。身份层仍只声明稳定能力与事实优先原则，创作方法论继续由 `apps/agents/.agents/skills/tapcanvas-*` 按需加载。
- 浏览器会话门禁在 TapCanvas 侧保持关闭：`HostConnectionService` 的 Host/Origin 信任围栏与精确路由注册表照常生效，`BrowserAuth` 是显式 no-op，不写 `credentials` 会话记录、不签发 Cookie、不接受 `?token=` 交换。上游新增的 `recovery` 连接恢复配置随插件默认值生效。
- 嵌入模式布局跟随上游三列改造：原生 details 列已由 rightbar 列取代，右侧列宽按上游 `rightbar` 计算；嵌入时左侧导航轨道仍固定为 `0px` 且不提供重新展开入口，TapCanvas 顶部 Agent 工具栏仍是唯一默认入口。
- 会话持久化格式跟随上游到 v3 并收紧历史格式迁移校验；`apps/agents` 内不再保留 0.1.x 时期残留的 v1 `session.jsonl` 夹具，历史会话读取继续只由 Harness JSONL 承担。上游同时把 `native/landlock-run` 工作区替换为 `native/system`。
...
- `POST /public/agent-api/video-jobs` 还接受可选的 `videoResolution`、`aspectRatio` 与 `targetDurationSeconds`。目标时长是最终成片总时长，必须为 1–180 秒；超过 180 秒在协议校验阶段显式拒绝，禁止自动截短。长片由统一视频工作流依据所选模型的实时单 clip 合法档位拆段并经后端合成，不能把 180 秒冒充成单次供应商请求时长。显式值在受理时冻结并优先于账号偏好；省略分辨率或画幅时冻结账号最近选择，新账号初始值为 `qwen-image-2.1 / 1K` 与 `minimax-h3 / 768p / 16:9`。同一份冻结事实与固定 `maxVideoDurationSeconds=180` 进入 BullMQ job、Agent API 需求节点与主 Agent 请求，必须逐字写入生成合同并以本轮实时模型目录验证；不支持时显式失败，禁止按目录顺序、模型家族或供应商错误自动改规格、换模型。队列只保存这些非密钥事实，不保存 API Key 明文。
...
独立旁白与音乐由唯一的服务端音频节点执行器 `generateAudioToCanvas`（`agents-tool-bridge.generate-audio-to-canvas.ts`）完成：原生 Agent 的 `tapcanvas_audio_generate_to_canvas`（影视别名 `film_audio_gen`）与持久工作流 runner 共用这一份实现和同一节点结构 `kind=audio + text + audioType + audioModel`，成功后画布节点携带真实 `audioUrl/assetId/audioDurationSec` 与来源 `audioModelEngine`。`audioModel` 省略且 `audioType=speech` 时只按实时音频目录声明的 `tapcanvas:audio-engine=minimax-h3` 语音模型解析；目录未声明该能力或存在多个候选时显式失败，禁止回退到其它引擎或模型，`runtimeParameters` 只能来自所选目录项。参考音色只接受当前画布真实音频节点 ID（执行器自行读取其真实 `audioUrl`，最多 3 条），音乐生成不接受参考音频。Agent 仍可只通过 `tapcanvas_flow_patch` 写入 `kind=audio + prompt/text + audioType + enabledAudioModels 中的精确 audioModel` 的节点，交由 Web 复用与手动点击“生成”完全相同的 `CanvasService.runNode -> DAG -> runNodeAudio` 执行器补齐。无论走哪条提交路径，节点 readiness 都只按正文、精确模型、节点状态与真实 `audioUrl` 判定，不解析用户文案，也不把 mere node creation 冒充供应商受理或资产交付。

异步执行一旦产生带稳定 `nodeId/taskId/runId` 的结构化受理证据，agents-cli 默认把当前物理回合收口为 `waiting_for_evidence`，并保留 `requiresExecutionDelivery=true` 的执行交付合同。后续正文不能把同一逻辑任务降级成非执行文本交付；runtime 回灌真实受理证据并禁止重放业务动作。唯一例外是工具在完成父任务授权范围后显式返回 `completionBoundary="submission"`：此时同一回执成为 persisted-state 终态证据，父任务成功结束，子节点继续独立物化。异步证据身份由规范化的 `toolName/kind/state/nodeId/taskId/runId/clipIndex/completionBoundary` 与最强 durable source 计算，不使用物理窗口内的数组序号；同一 durable 状态即使由新的 tool call 重放也生成逐字段相同的证据并幂等去重，状态、任务、clip 或边界发生真实变化则生成新的 evidenceId 追加到 journal，绝不覆盖旧证据或放宽 append-only 约束。`tapcanvas_image_generate_to_canvas` 的单图与批量入口统一拒绝 `waitForResult:true`：供应商一旦返回 queued/running 和 taskId，Hono 必须先把同一 taskId 写入 running 节点再响应，由画布 SSE/reconcile 收取结果，禁止在持久化回执前进行分钟级 HTTP 轮询。图片与视频 reconcile 共用单一的供应商错误提取协议；上游终态失败时，图片节点必须保存有界的 `error/errorMessage/providerStatus`，并在 `details[].errorMessage` 返回相同诊断，禁止只留下无法行动的 `failed`。装备 Workflow IR 的 execution receipt 与代理直接受理的图片生成使用同一个通用异步证据投影，但只有后者当前声明提交即父任务完成；音频节点无论由服务端音频执行器还是浏览器节点执行器受理，都以画布节点状态、`taskId` 与真实 `audioUrl` 进入同一事实验收层，不存在脱离真实资产的受理证据。
...
执行合同。它可渲染自带 HTML 的确定性包装片段，也可接收已由 See-through 产生、具有真实 http(s) RGBA 部件 URL、
画布坐标、深度和关键帧的 `characterAnimation` 合同来生成二维角色 MP4；后者与 `html/assets` 互斥，服务端只重组真实
部件，不生成角色、场景或缺失图层。组件 URL、渲染器或对象存储缺失时显式失败，不能改走生成式视频。execution list 在 PROJECT+CANVAS 下是 direct 结构诊断；get/node-runs/events 还要求当前 envelope 已带
...
生成偏好（或用户本轮显式覆盖）进入 BeatSheet，再与实时 `enabledVideoModels` 冻结 generationContract；分段不再是持久偏好。新账号初始值固定为 `qwen-image-2.1 / 1K` 与 `minimax-h3 / 768p / 16:9`，不是“目录第一项”；Web 偏好弹窗的分辨率与画幅选项直接来自当前视频模型的实时目录合同，不维护第二套静态规格列表。任何初始值或最近选择不在实时可执行目录时都原地失败，不能自动选择另一个候选。
...
音频模型目录可通过 `tapcanvas:audio-engine=minimax-h3` 标签声明 MiniMax H3 执行端。Hono 从 `MINIMAX_H3_TTS_BASE_URL`（默认 `http://127.0.0.1:8188`）直连用户已运行的 ComfyUI：先以 `/object_info` 校验 H3 UNET、文本编码器与 VAE 的实时枚举，参考音频再上传到 `/upload/image`，随后向 `/prompt` 提交真实 H3 节点图、轮询 `/history/{prompt_id}`，最后通过 `/view` 取回音频并登记为项目资产；不启动也不依赖额外的 `8199 /generate` 封装服务。H3 音频没有任何对外暴露的模型参数：时长不是用户输入，节点与 `/public/audio/speech` 都不接受 `duration`，服务端一律按提示词里的 `<d>` 台词字符数（中文约 4.5 字/秒）加开场、句间停顿与收尾空间，并以 `At MM:SS.mmm` 时间轴预算取较大值，硬限制 1~15 秒，估算超过 15 秒则明确拒绝并要求精简台词或拆成多条音频节点；采样步数与可用 UNET 同样取 H3 工作流固定配置（`steps=10`、`scheduler=beta`）并由执行前对 `/object_info` 的实时枚举校验，节点不暴露 `steps`/`unet`，请求也不接受这两个字段。因此该模型在目录中不声明任何 `meta.runtimeParameters`，节点只显示模型与能力标签。短音频的对白起声时序属于 H3 Prompt 合同：首镜不得在对白前堆叠动作，第一句需写为第二镜的立即起声动作。交付阶段会按文档第 6 与第 10 节保留原始 FLAC 作为来源资产，并固定移除 Prompt 合同中“首秒无人声”的预卷，再仅裁起始连续静音；随后加入 50ms 前导静音，避免播放器在 0 秒硬切入人声。尾部不使用响度阈值自动裁剪，确保所有后续声音完整保留。`durationSec`、节点 `audioDurationSec` 与音频结果时长均来自处理后 WAV 的 `ffprobe` 实测，原始 URL/时长另记为 `audioSourceUrl`/`audioSourceDurationSec`。未配置服务地址、模型目录标签不匹配、参考资产不可读、ComfyUI 节点或模型不匹配时同样显式失败，不会静默切换到其他 TTS 引擎。
