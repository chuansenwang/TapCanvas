...
func TaskPollingLoop() {
	for {
		...
		// 每轮设置独立的超时 context，防止 DB/HTTP 卡死导致轮询永久堵塞。
		timeoutSec := constant.TaskPollTimeoutSeconds
		if timeoutSec <= 0 {
			timeoutSec = 600
		}
		...
		for platform, tasks := range platformTask {
			...
			// 若整轮已超时则提前退出，避免继续执行堆积的平台
			if ctx.Err() != nil {
				logger.LogError(ctx, fmt.Sprintf("任务轮询超时，剩余平台 %v 跳过", platform))
				break
			}
...
func updateSunoTasks(ctx context.Context, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	maxBytes := int64(constant.TaskResponseMaxMB) * 1024 * 1024
	if maxBytes <= 0 {
...
//
//  2. 普通按次任务无 usage 调整 → 保持预扣额度
//  3. taskResult.TotalTokens > 0 → 按 token 重算
//  4. 都不满足 → 保持预扣额度不变
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) {
	// 1. 优先让 adaptor 根据上游真实 usage 决定最终额度。按次模型也可能在
	// 异步完成后才返回输入视频秒数/图片数，因此不能在调用 adaptor 前跳过。
	if actualQuota := adaptor.AdjustBillingOnComplete(task, taskResult); actualQuota > 0 {
		RecalculateTaskQuota(ctx, task, actualQuota, "adaptor计费调整")
		return
	}
	// 2. 普通按次计费任务没有 usage 调整时保持预扣额度。
	if bc := task.PrivateData.BillingContext; bc != nil && bc.PerCallBilling {
		logger.LogInfo(ctx, fmt.Sprintf("任务 %s 按次计费，保持预扣额度", task.TaskID))
		return
	}
	// 3. 回退到 token 重算
