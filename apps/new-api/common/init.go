...
func initConstantEnv() {
	...
	// relay 全局最大并发请求数，超出时返回 429。0 表示不限制。
	constant.MaxRelayConcurrency = GetEnvOrDefault("MAX_RELAY_CONCURRENCY", 300)
	// 任务轮询单次循环超时（秒），防止 DB/HTTP 卡死导致轮询堆积。
	constant.TaskPollTimeoutSeconds = GetEnvOrDefault("TASK_POLL_TIMEOUT_SECONDS", 600)
	// 任务轮询时单个响应体的最大大小（MB），防止大响应 OOM。
	constant.TaskResponseMaxMB = GetEnvOrDefault("TASK_RESPONSE_MAX_MB", 32)
