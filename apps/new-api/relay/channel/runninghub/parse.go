...
// Normalized terminal states. RunningHub uses QUEUED/RUNNING/SUCCESS/FAILED, but
// parseTaskResult maps any of the common conventions onto these so the channel is
// resilient to upstream wording changes.
const (
...
// extractTaskID pulls the task id out of the submit envelope. RunningHub returns
// {"taskId":"..."}; we scan for the first plausible id field at any depth
// (taskId / task_id / id), tolerating both string and numeric values.
func extractTaskID(body []byte) (string, error) {
	var root any
	if err := common.Unmarshal(body, &root); err != nil {
		return "", fmt.Errorf("runninghub: unmarshal submit body failed: %w, body=%s", err, string(body))
	}
	...
	return "", errors.New("runninghub: no task id in submit response")
}

// parseTaskResult returns the normalized status and any result image URLs found
// in a submit/query response. RunningHub puts URLs at results[].url; URL
// extraction is recursive and key/shape agnostic, so it survives schema changes
// as long as the result URLs are http(s) strings.
...
// message fields (errorMessage / failedReason / message / msg / error).
func extractFailureReason(body []byte) string {
	...
	return findFirstByKeys(root, map[string]struct{}{
		"errormessage": {}, "failedreason": {}, "failreason": {}, "failurereason": {},
		"errmsg": {}, "errormsg": {}, "message": {}, "msg": {}, "error": {}, "reason": {},
	})
}

// --- generic JSON walkers ---

// findFirstByKeys returns the first non-empty string/number value whose
// (lowercased) key is in keys, searching depth-first.
...
func normalizeStatus(raw string) string {
	...
	case "":
		return ""
	default:
		// QUEUED / RUNNING / PENDING / PROCESSING / 0 / 1 …
