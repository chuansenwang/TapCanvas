...
//
// Migrated from sub2api (internal/service/claude_code_validator.go)。去除 ctxkey
// 依赖：原本从 context 读取的 "max_tokens=1 haiku 探测" 信号改为 Validate 的入参。
...
const (
	...
	// 该块存在于所有真实 Claude Code CLI 请求中，格式固定、不随提示词改版漂移，
	// 是比身份 prose 更稳定的客户端标识。
...
//
// 采用与 claude-relay-service 一致的验证策略：
//
//	Step 1: User-Agent 检查 (必需) - 必须是 claude-cli/x.x.x
//	Step 2: 对于非 messages 路径和 /messages/count_tokens，只要 UA 匹配就通过
//	Step 3: 检查 max_tokens=1 + haiku 探测请求绕过（由入参 maxTokensOneHaiku 传入）
//	Step 4: 对于 messages 路径，进行严格验证
//
// maxTokensOneHaiku 来自调用方：原 sub2api 从 context 读取，这里改为显式入参以去除
// ctxkey 依赖。
func (v *ClaudeCodeValidator) Validate(r *http.Request, body map[string]any, maxTokensOneHaiku bool) bool {
...
// ExtractCLIVersion 从 User-Agent 中提取 Claude Code 版本号。
// 返回 "" 如果 UA 不匹配预期格式。
//
// Migrated from sub2api (internal/service/metadata_userid.go)。
...
func ParseMetadataUserID(raw string) *ParsedUserID {
	...
	if raw[0] == '{' {
		var j jsonUserID
		if err := common.UnmarshalJsonStr(raw, &j); err != nil {
			return nil
		}
