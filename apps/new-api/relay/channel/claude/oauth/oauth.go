// Package oauth provides helpers for the Claude subscription OAuth flow.
//
// Migrated from sub2api (internal/pkg/oauth/oauth.go). Pure protocol logic with
// no internal coupling: client_id / authorize+token URLs / scopes / PKCE /
// BuildAuthorizationURL / OAuthSession+SessionStore / token response structs.
package oauth
...
// TokenEndpointForBase 返回 OAuth token 端点。baseURL（渠道 base_url）非空时拼接
// /v1/oauth/token——该路径在 api.anthropic.com 与 platform.claude.com 均存在，
// 走反代域名的渠道（部署地无法直连 Anthropic）刷新与转发同路；为空回落官方端点。
func TokenEndpointForBase(baseURL string) string {
	b := strings.TrimSpace(baseURL)
	if b == "" {
		return TokenURL
	}
	return strings.TrimSuffix(b, "/") + "/v1/oauth/token"
}
