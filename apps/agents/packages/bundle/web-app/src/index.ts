...
function resolveDistIndex(): string {
  ...
  if (tapCanvasDistIndex !== undefined && tapCanvasDistIndex !== '') {
    if (!isAbsolute(tapCanvasDistIndex) || resolve(tapCanvasDistIndex) !== tapCanvasDistIndex
      || !tapCanvasDistIndex.endsWith(`${join('dist', 'index.html')}`)) {
      throw new Error('web-app: TAPCANVAS_WEB_DIST_INDEX must be an absolute apps/web/dist/index.html path')
    }
